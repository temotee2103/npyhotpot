...
function AdminDeliveryDiscountEditPageContent() {
  ...
  useEffect(() => {
    if (!id) return;
    let active = true;
    fetchOfficialDiscountById(id, "delivery").then((data) => {
    ...
    <div className="space-y-5">
      <section className="rounded-2xl border border-primary/10 bg-white p-5 shadow-sm dark:border-primary/20 dark:bg-slate-900/70">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-bold tracking-[0.14em] text-primary">COUPON RULE</p>
          ...
          <div className="flex gap-2">
            <Link
              href={`/admin/delivery/discounts?channel=${channel}`}
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (!targetId) {
                  ...
                  const created = await createOfficialDiscountDraft("delivery");
                  if (!created) {
                    setSaving(false);
                    setSaved("创建失败，请重试");
                    return;
                ...
                router.push(`/admin/delivery/discounts?channel=${channel}&t=${Date.now()}`);
              }}
          ...
          <div className="mt-4 space-y-3">
            <div className="grid gap-2 sm:grid-cols-2">
              ...
              >
                ...
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-300">仅支持 MYR</p>
              </button>
            </div>
      ...
      <AdminConfirmModal
        ...
        onConfirm={async () => {
          ...
          setDeleting(true);
          setSaved(null);
          const result = await deleteOfficialDiscount(id, "delivery");
          setDeleting(false);
          if (!result.ok) {
...
export default function AdminDeliveryDiscountEditPage() {
  return (
    <Suspense fallback={null}>
      <AdminDeliveryDiscountEditPageContent />
    </Suspense>
  );
}
