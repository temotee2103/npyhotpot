...
function AdminShopDiscountsPageContent() {
  ...
  useEffect(() => {
    let active = true;
    fetchOfficialDiscounts("shop").then((data) => {
      if (!active) return;
      setRows(data);
    });
    return () => {
    ...
    <div className="space-y-5">
      <section className="rounded-2xl border border-primary/10 bg-white p-5 shadow-sm dark:border-primary/20 dark:bg-slate-900/70">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-3xl font-black">优惠券管理</h1>
          ...
          <button
            ...
            onClick={async () => {
              ...
              setCreating(true);
              setCreating(false);
              router.push(`/admin/shop/discounts/edit?new=1&channel=${channel}`);
            }}
                ...
                <tr key={discount.id} className="border-t border-slate-100 dark:border-slate-800">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex flex-wrap gap-2">
                      <Link
                        href={`/admin/shop/discounts/edit?id=${discount.id}&channel=${channel}`}
      ...
      <AdminConfirmModal
        ...
        onConfirm={async () => {
          ...
          const result = await deleteOfficialDiscount(confirmTarget.id, "shop");
          setDeletingId(null);
          if (!result.ok) return;
