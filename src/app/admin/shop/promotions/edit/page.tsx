...
function toTimeOptions(current: string) {
  if (!current) return FIXED_TIME_OPTIONS;
  if (!/^\d{2}:\d{2}$/.test(current)) return FIXED_TIME_OPTIONS;
  if (FIXED_TIME_OPTIONS.includes(current)) return FIXED_TIME_OPTIONS;
  return [...FIXED_TIME_OPTIONS, current].sort();
}

function AdminShopPromotionEditPageContent() {
  ...
  useEffect(() => {
    ...
    fetchOfficialPromotionById(id).then((data) => {
      ...
      setDailyStart(data.daily_start ?? "");
      setDailyEnd(data.daily_end ?? "");
  ...
  const dailyStartOptions = useMemo(() => toTimeOptions(dailyStart), [dailyStart]);
  const dailyEndOptions = useMemo(() => toTimeOptions(dailyEnd), [dailyEnd]);
    ...
    <div className="space-y-5">
      <section className="rounded-2xl border border-primary/10 bg-white p-5 shadow-sm dark:border-primary/20 dark:bg-slate-900/70">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <Link
              href={`/admin/shop/promotions?channel=${channel}`}
      ...
      <section className="rounded-2xl border border-primary/10 bg-white p-5 shadow-sm dark:border-primary/20 dark:bg-slate-900/70">
        ...
        {scheduleKind === "range" ? (
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-300">每日开始（固定时间）</p>
              <select
                value={dailyStart}
            ...
            <div>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-300">每日结束（固定时间）</p>
              <select
                value={dailyEnd}
...
export default function AdminShopPromotionEditPage() {
  return (
    <Suspense fallback={null}>
      <AdminShopPromotionEditPageContent />
    </Suspense>
  );
}
