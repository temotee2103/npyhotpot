...
function AdminShopPromotionsPageContent() {
  ...
  useEffect(() => {
    let active = true;
    fetchOfficialPromotionsByChannel("shop").then((data) => {
      if (!active) return;
      setRows(data);
    });
    return () => {
      ...
      <section className="rounded-2xl border border-primary/10 bg-white p-5 shadow-sm dark:border-primary/20 dark:bg-slate-900/70">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-300">独立于外卖的商城促销模块（仅影响商城前台展示）。</p>
          </div>
          <div className="flex gap-2">
            ...
            <button
              ...
              onClick={async () => {
                if (creating) return;
                setCreating(true);
                const created = await createOfficialPromotionDraft("shop");
                setCreating(false);
                if (!created) return;
                router.push(`/admin/shop/promotions/edit?id=${created.id}&channel=${channel}`);
              }}
              ...
              {(rows ?? []).map((item) => (
                <tr key={item.id} className="border-t border-slate-100 dark:border-slate-800">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex gap-2">
                      <Link
                        href={`/admin/shop/promotions/edit?id=${item.id}&channel=${channel}`}
...
export default function AdminShopPromotionsPage() {
  return (
    <Suspense fallback={null}>
      <AdminShopPromotionsPageContent />
    </Suspense>
  );
}
