revoke execute on function public.official_issue_coupon_to_user(uuid, uuid, uuid, text, uuid, numeric, timestamptz, jsonb) from authenticated;
...
declare
  ...
  v_legacy_points numeric := 0;
  v_unified_points numeric := 0;
  v_coupon_id uuid;
begin
  ...
  select coalesce(sum(points_delta), 0)
  into v_unified_points
  from public.official_points_ledger
  where user_id = v_user_id;

  v_points_balance := v_legacy_points + v_unified_points;
